...
public class BoardController {
    ...
    private Map<Object, Consumer<Pair<String,Tag>>> listeners = new HashMap<>();
    ...
    public ResponseEntity<Tag> addBoardTag(@PathVariable("key") String key, @RequestBody Tag tag) {
        try {
            ...
            Pair<String, Tag> myPair = Pair.of("Add", updateBoard.getTagList().get(updateBoard.getTagList().size()-1));
            listeners.entrySet().stream().filter(entry -> entry.getKey().equals(key))
                    .map(Map.Entry::getValue).forEach((l) -> l.accept(myPair));
    ...
    public ResponseEntity<Tag> removeBoardTag(@PathVariable("key") String key, @RequestBody Tag tag) {
        try {
            ...
            service.insert(updateBoard);
            Pair<String, Tag> myPair = Pair.of("Remove",tag);
            listeners.entrySet().stream().filter(entry -> entry.getKey().equals(key))
                    .map(Map.Entry::getValue).forEach((l) -> l.accept(myPair));
    ...
    public DeferredResult<ResponseEntity<Pair<String,Tag>>> getTagUpdates(@PathVariable("key") String key) {
        ...
        var k = new Object();
        listeners.put(k, t -> res.setResult(ResponseEntity.ok(t)));
        res.onCompletion(() -> {
            listeners.remove(k);
        });
